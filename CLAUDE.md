...
Browser-based stroke-painting renderer implementing four stroke-based NPR algorithms (Hertzmann 1998 curved strokes, Hertzmann 2001 Paint By Relaxation, Litwinowicz 1997 impressionist strokes, Secord 2002 weighted Voronoi stippling) plus the neural Paint Transformer (ICCV 2021). Scope is deliberately the curved-stroke family: the colored-pencil, Haeberli paint-by-numbers and Shiraishi moment-fitted styles were removed. Runs entirely client-side — no server, no build step.
...
| `styles/relaxation.js` | Hertzmann 2001 Paint By Relaxation — energy-driven stroke placement |
...
| `relaxation` | `paintRelaxation` (styles/relaxation.js) | Hertzmann 2001: same curved strokes, but placement minimizes `E = SUM_pixels ||Lab(canvas)-Lab(ref)|| + w_area * area`. A candidate's energy delta is evaluated exactly over its footprint and it is painted only if `dE < 0`, so `relaxAreaWeight` reads directly as "minimum mean Lab improvement per unit area". Several passes per layer = coordinate descent; `relaxTrials` candidates per cell approximate the paper's relocation. Treats `underpaintMode: 'blur'` as `'average'` (a blurred start is already near the energy minimum and suppresses every stroke). Cost scales with `relaxPasses * relaxTrials` — the search evaluates ~100 candidates per stroke it paints — so those two are the knobs that decide render time; `relaxPasses` defaults to 1 because the second pass bought 5% more strokes for 48% of the evaluations. |
...
RNG & determinism: every algorithm is seeded — same `params.seed` + same params ⇒ byte-identical output. Each algorithm creates one `mulberry32(CONSTANT ^ (params.seed | 0))` per render and threads it through every random draw (including `finalizeStrokeColor`'s jitters — never let it fall back to its `Math.random` default). Seeds are stable **within** a version only: adding or reordering any RNG draw silently changes fixed-seed output — when that happens intentionally, regenerate the parity baselines (below). Fixed constants per algorithm: hertzmann `0x1E52A11`, relaxation `0x9A17E5`, litwinowicz `0xC0FFEE`, stipple `0x577DD1E`, neural color jitter `0x7A1D7E`.
...
**Direction fields**: `strokeDirectionField(rgb, w, h, params)` picks, in order, `computeGradientsETF` (Kang, Lee & Chui 2007 Edge Tangent Flow — iterative, keeps strokes running *along* edges instead of smearing direction across them; separable per-iteration approximation of the paper's disc neighbourhood), `computeGradientsST` (single-pass structure tensor), or raw Sobel. ETF returns the same `{gx, gy, gmag}` shape — the tangent is rotated back into gradient space — so callers are unchanged. `etfRadius > 0` forces the CPU map path in GPU mode (no ETF shader).

**Video export invariants** (`video-batch.js`): frame timestamps come from the output fps grid (`i * 1e6 / fps`), never from the decoded `mediaTime` — the muxer writes block timecodes straight from them at 1 ms resolution, so non-grid timestamps produce irregular frame durations, i.e. playback hitching. Extraction is seek-and-draw only; do not reintroduce `play()`/`requestVideoFrameCallback` to read a frame, because `drawImage` then runs at a different moment than the callback and the pixels can belong to a later frame than the timestamp claims. `PainterWorker` holds one `_pending` slot, rejects it on `terminate()` (otherwise cancel hangs the export forever), and refuses overlapping renders rather than silently handing one frame's result to another caller. `tools/video-check.html` regression-checks order and cadence.
